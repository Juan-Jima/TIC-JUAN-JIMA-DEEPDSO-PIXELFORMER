...
void CoarseInitializer::setDepthFromExternal(const cv::Mat& depth_ext)
{
    if(depth_ext.empty() || depth_ext.type() != CV_32F)
    {
        printf("WARNING: Invalid external depth map\n");
        return;
    }
    
    if(depth_ext.rows != h[0] || depth_ext.cols != w[0])
    {
        printf("WARNING: Depth map size mismatch: %dx%d vs %dx%d\n", 
               depth_ext.cols, depth_ext.rows, w[0], h[0]);
        return;
    }
    ...
    for(int i=0; i<numPoints[0]; i++)
    {
        ...
        {
            float d = depth_ext.at<float>(v, u);
            
            // Validar rango de profundidad (0.15m - 50m)
            if(d > 0.15f && d < 50.0f)
            {
                // Establecer inverse depth (1/d)
                float inv_d = 1.0f / d;
                point->iR = inv_d;
                point->idepth = inv_d;
                point->idepth_new = inv_d;
                pointsUpdated++;
            }
        }
    }
    
    printf("✅ External depth applied to %d/%d points (range: %.2f-%.2f m)\n", 
           pointsUpdated, numPoints[0], 0.15f, 50.0f);
}
