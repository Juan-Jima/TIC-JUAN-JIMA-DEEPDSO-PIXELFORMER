...
#include "FullSystem/DepthCommunicator.h" 
#include "stdio.h"
...
void FullSystem::initializeFromInitializer(FrameHessian* newFrame)
{
	...
	// ============== INTEGRACIÓN PIXELFORMER ==============
	printf("🔍 Solicitando depth map a PixelFormer...\n");
	
	// Convertir imagen a OpenCV Mat (BGR)
	cv::Mat inputImage(hG[0], wG[0], CV_8UC3);
	for(int y = 0; y < hG[0]; y++)
	{
		for(int x = 0; x < wG[0]; x++)
		{
			int idx = x + y * wG[0];
		unsigned char gray = (unsigned char)(firstFrame->dI[idx][0] * 255.0f);
		inputImage.at<cv::Vec3b>(y, x) = cv::Vec3b(gray, gray, gray);
		}
	}
	
	// Solicitar depth map al servidor
	cv::Mat depthMap = DepthCommunicator::requestDepthMap(inputImage);
	
	if(!depthMap.empty())
	{
		printf("✅ Depth map recibido, aplicando a inicializador...\n");
		coarseInitializer->setDepthFromExternal(depthMap);
	}
	else
	{
		printf("⚠️  No se pudo obtener depth map, continuando sin él\n");
	}
	// ====================================================
